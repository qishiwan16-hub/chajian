...
- `[无更新]`：本地已经是最新
- `[已更新]`：已经成功拉取远程更新
- `[跳过]`：不是 Git 仓库、没有配置上游分支，或仓库已转私有、无权限访问、地址失效、需要登录认证
- `[失败]`：这个扩展更新失败，但脚本会继续检查下一个

如果某个扩展仓库已经转私有、你当前无权限访问、仓库地址失效，或访问时需要登录认证，脚本会自动跳过，不会卡住整轮更新；这类情况会记入“跳过”，不会记入“失败”。
...
### 3. 为什么有些扩展被跳过
通常是因为那个目录不是 Git 仓库、没有配置上游分支，或者对应仓库已转私有、无权限访问、地址失效、需要登录认证。

如果你后面已经处理好权限、认证或仓库地址，可以进入对应扩展目录后手动执行：
